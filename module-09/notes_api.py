...
def list_notes(q: Optional[str] = Query(default=None, min_length=1)) -> list[Note]:
    with get_conn() as conn:
        if q:
...
def update_note(note_id: int, note: NoteUpdate) -> Note:
    ...
    values = [*updates.values(), ts, note_id]
    with get_conn() as conn:
        row = conn.execute(
            f"UPDATE notes SET {set_clause}, updated_at = ? WHERE id = ? RETURNING *",
            values,
        ).fetchone()
    if row is None:
        raise HTTPException(status_code=404, detail="not found")
